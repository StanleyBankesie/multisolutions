...
function refactorPaymentVoucher() {
  ...
  if (code.includes('async function loadOutstandingBillsForSupplier(accountCode) {')) {
    code = code.replace(loadFuncOld, loadFuncNew);
  }
  ...
  if (code.includes('if (isPAYV && paymentType === "AGAINST_BILL" && pvForm.payToCode) {')) {
    code = code.replace(useEffectOld, useEffectNew);
  }
  
  // Also refactor the one in `async function save(status = "DRAFT")` ? Wait, there is one more call to loadOutstandingBillsForSupplier in save? No.
...
function refactorReceiptVoucher() {
  ...
  if (code.includes('async function loadInvoicesForCustomer(accountId) {')) {
    code = code.replace(loadFuncOld, loadFuncNew);
  }
  ...
  if (code.includes('if (isRV && receiptType === "AGAINST_INVOICE" && rvForm.receivedFromAccountId) {')) {
    code = code.replace(useEffectOld, useEffectNew);
  }
